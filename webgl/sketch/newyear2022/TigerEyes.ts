...
export default class TigerEyes extends THREE.Mesh {
  ...
  update(time: number) {
    ...
    const stepShow1 = easing.outCirc(MathEx.clamp((this.timeShow - DELAY) / DURATION, 0, 1))
    const stepShow2 = easing.outExpo(MathEx.clamp((this.timeShow - DELAY) / DURATION, 0, 1))
