...
void main(void) {
  vec2 transformedUV = uv * 1.0 / 6.0 + vec2(alphaIndex / 6.0, time * 0.03);

  vec3 transformed = position;
  vec4 noise = texture2D(tNoise, transformedUV);
  transformed += vec3(
    cos(noise.r * radians(360.0)) * 3.0,
    sin(noise.g * radians(360.0)) * 3.0,
    (noise.b * 2.0 - 1.0) * 12.0 + floor(alphaIndex / 2.0) * (mod(alphaIndex, 2.0) * 2.0 - 1.0) * 6.0
